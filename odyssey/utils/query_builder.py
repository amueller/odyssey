...
For example, for the where clause, if there are multiple constraints, we can connect them using connect_with_and / connect_with_or defined in this file.
...
def connect(connector, *args):
	...
	if len(args) == 1 and args[0] == tuple([]):
		return ""
	L = ["(" + val + ")" for val in args if val.strip() != ""] # remove empty values
	if len(L) == 0:
		return ""
	elif len(L) == 1:
		return L[0]
